...
import { AnalyticsJob, JobStatus } from './types'
import { QUEUES } from '../lib/queue'

export async function analyticsAggregator(fastify: FastifyInstance): Promise<Worker> {
...
async function processViewEvent(
  fastify: FastifyInstance,
  event: { videoId?: string; userId?: string; data: any; timestamp: Date }
) {
...
async function processRetentionEvent(
  fastify: FastifyInstance,
  event: { videoId?: string; data: any; timestamp: Date }
) {
  if (!event.videoId || !event.data.retentionData) return
...
async function aggregateDailyStats(fastify: FastifyInstance) {
  try {
    const today = new Date()
    today.setHours(0, 0, 0, 0)
    ...
    for (const creator of creators) {
      const dailyViews = creator.videos.reduce(
        (sum, video) => sum + video._count.viewEvents,
        0
      )
