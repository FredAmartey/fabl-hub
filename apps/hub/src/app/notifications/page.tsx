  ...
  return (
    <div className="px-6 pt-6 pb-12 max-w-4xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-gray-400 mt-1">Stay updated with the latest activity</p>
          </div>
        </div>
        <Button variant="ghost" size="sm" onClick={() => setSelectMode(!selectMode)}>
      ...
      <div className="flex overflow-x-auto space-x-2 mb-6 pb-2">
        ...
        <button
          className={`px-3 py-1.5 rounded-full text-sm whitespace-nowrap transition-colors flex items-center ${
            filter === "like"
              ? "bg-gradient-to-r from-purple-600 to-blue-500 text-white"
              : "bg-[#1a1230] hover:bg-purple-500/30 text-gray-300"
          }`}
      ...
      {/* Selection Controls */}
      {selectMode && (
        <div className="flex items-center justify-between mb-4 p-3 bg-[#1a1230] rounded-lg">
          <div className="flex items-center">
            <Checkbox
              checked={
      ...
      <div className="space-y-3">
        {filteredNotifications.length > 0 ? (
          filteredNotifications.map((notification) => (
            ...
            >
              ...
              <div className="w-10 h-10 rounded-full overflow-hidden mr-3 flex-shrink-0">
                <Link href={`/channel/${notification.user.id}`}>
                  <img
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <div className="font-medium">
                      <Link
                        href={`/channel/${notification.user.id}`}
                    ...
                    {notification.comment && (
                      <div className="mt-1 text-sm text-gray-300 bg-[#0f0a1e] p-2 rounded">
                        &quot;{notification.comment}&quot;
                      </div>
                    )}
                    {notification.video && (
                      <Link
                        href={`/video/${notification.video.id}`}
                        className="mt-2 flex items-start hover:bg-[#0f0a1e] p-1 rounded transition-colors"
                      >
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="ml-2">{getNotificationIcon(notification.type)}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="text-center py-16 bg-[#1a1230] rounded-xl">
