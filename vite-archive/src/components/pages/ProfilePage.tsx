...
import { UserIcon, SettingsIcon, EditIcon, VideoIcon, BookmarkIcon, ThumbsUpIcon, BellIcon, ShareIcon, UsersIcon } from 'lucide-react';
...
export function ProfilePage() {
  ...
  const tabContent = {
    videos: <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {userVideos.map(video => <VideoCard key={video.id} video={video} />)}
      </div>,
    playlists: <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {playlists.map(playlist => <Link key={playlist.id} to={`/playlist/${playlist.id}`} className="group relative block">
    ...
    about: <div className="bg-[#1a1230] rounded-xl p-6">
    ...
    liked: <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      ...
      <div className="px-6">
        <div className="flex flex-col md:flex-row items-start md:items-end -mt-16 md:-mt-20 relative z-10 mb-6">
          <div className="w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-[#0f0a1e] bg-[#0f0a1e] mr-4">
            {user.avatar ? <img src={user.avatar} alt={user.name} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gradient-to-br from-purple-500 to-blue-400 flex items-center justify-center">
        ...
        <div className="border-b border-gray-700 mb-6">
          <div className="flex overflow-x-auto">
            <button className={`px-4 py-2 font-medium text-sm border-b-2 transition-colors ${activeTab === 'videos' ? 'border-purple-500 text-white' : 'border-transparent text-gray-400 hover:text-white'}`} onClick={() => setActiveTab('videos')}>
              <VideoIcon className="w-4 h-4 inline-block mr-1" />
              Videos
            </button>
            <button className={`px-4 py-2 font-medium text-sm border-b-2 transition-colors ${activeTab === 'playlists' ? 'border-purple-500 text-white' : 'border-transparent text-gray-400 hover:text-white'}`} onClick={() => setActiveTab('playlists')}>
              <div className="w-4 h-4 inline-block mr-1" />
              Playlists
            </button>
            <button className={`px-4 py-2 font-medium text-sm border-b-2 transition-colors ${activeTab === 'liked' ? 'border-purple-500 text-white' : 'border-transparent text-gray-400 hover:text-white'}`} onClick={() => setActiveTab('liked')}>
              <ThumbsUpIcon className="w-4 h-4 inline-block mr-1" />
              Liked
            </button>
            <button className={`px-4 py-2 font-medium text-sm border-b-2 transition-colors ${activeTab === 'about' ? 'border-purple-500 text-white' : 'border-transparent text-gray-400 hover:text-white'}`} onClick={() => setActiveTab('about')}>
