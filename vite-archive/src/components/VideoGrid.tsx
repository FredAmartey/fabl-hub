...
export function VideoGrid() {
  const categories = ['All', 'AI Cinematics', 'Neural Dreams', 'Synthetic Stories', 'Visual Wonders', 'AI Music', 'Digital Art', 'Robot Creations'];
      ...
      <div className="relative mb-6">
        <div className="flex items-center">
          ...
          <div className="flex space-x-2 overflow-x-auto pb-2 scrollbar-hide">
            {categories.map(category => <button key={category} onClick={() => setActiveCategory(category)} className={`
                  px-4 py-2 rounded-full text-sm whitespace-nowrap transition-all
                  ${activeCategory === category ? 'bg-gradient-to-r from-purple-600 to-blue-500 text-white font-medium' : 'bg-[#1a1230] hover:bg-purple-500/30 text-gray-300'}
                `}>
                {category}
              </button>)}
          </div>
